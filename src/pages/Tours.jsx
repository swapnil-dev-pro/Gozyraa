...
const Tours = () => {
  ...
  const getGridColumns = () => {
    if (windowWidth < 640) return "1fr"; // mobile: 1 column
    if (windowWidth < 1024) return "repeat(2, 1fr)"; // tablet: 2 columns
    return "repeat(3, 1fr)"; // desktop: 3 columns (আগের মতো)
  };

  const getPadding = () => {
    if (windowWidth < 640) return "20px 16px";
    if (windowWidth < 1024) return "30px 32px";
    return "40px 60px";
  };
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <div
        style={{ padding: getPadding(), maxWidth: "1200px", margin: "0 auto" }}
      >
        {/* Title */}
        <h1
          style={{
